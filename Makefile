...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/opentsdb_encoderl
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/opentsdb_encoderl.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = opentsdb_encoderl-$(VERSION)
NAME_UPPER  := $(shell echo opentsdb_encoderl | awk '{print toupper($$1)}')
...
compile:
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/opentsdb_encoderl/ebin $(CURDIR)
...
shell:
	...
	$(PRE) \
        ...
        erl     -pa `ls -d _build/default/lib/*/ebin` \
                ...
                -eval "begin application:load('opentsdb_encoderl'), catch code:load_file('opentsdb_encoderl') end" \
                +B
...
docker:
	$(PRE) docker build -t opentsdb_encoderl ./ $(POST)
